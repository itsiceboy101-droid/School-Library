...
studentsRouter.get("/search", async (req: Request, res: Response) => {
  try {
    ...
    if (cls) {
      allStudents = allStudents.filter(
        (s) =>
          String(s.class || "")
            .trim()
            .toLowerCase() === String(cls).trim().toLowerCase(),
      );
    }
    if (division) {
      allStudents = allStudents.filter(
        (s) =>
          String(s.division || "")
            .trim()
            .toUpperCase() === String(division).trim().toUpperCase(),
      );
    }
    if (roll_no) {
      const targetRoll = String(roll_no).replace(/\D/g, "");
      allStudents = allStudents.filter(
        (s) => String(s.roll_no || "").replace(/\D/g, "") === targetRoll,
      );
    }
    if (card) {
      const qCard = String(card).toLowerCase().trim();
      allStudents = allStudents.filter((s) =>
        String(s.library_card_no || "")
          .toLowerCase()
          .includes(qCard),
      );
    }
    ...
    allIssuedBooks.forEach((ib) => {
      if (!issuedByStudent.has(ib.student_id))
        issuedByStudent.set(ib.student_id, []);
    ...
    const results = allStudents.map((s) => {
      ...
      const activeIssues = studentIssues.filter(
        (ib: any) => ib.status !== "returned",
      );
      const activeOverdue = activeIssues.find(
        (ib: any) => ib.due_date && ib.due_date < today,
      );
      ...
      if (activeOverdue) {
        ...
        const returnedLate = studentIssues.filter(
          (ib: any) =>
            ib.status === "returned" &&
            ib.return_date &&
            ib.due_date &&
            ib.return_date > ib.due_date,
        );
        if (returnedLate.length > 0) {
          ...
          if (today < banUntilStr) {
            if (
              s.manual_ban_lift_date &&
              s.manual_ban_lift_date >= mostRecentLate.return_date
            ) {
            } else {
              isRestricted = true;
...
studentsRouter.get("/", async (req: Request, res: Response) => {
  try {
    ...
    allStudents.sort((a, b) => {
      ...
      const rollA =
        parseInt(String(a.roll_no || "").replace(/\D/g, ""), 10) || 0;
      const rollB =
        parseInt(String(b.roll_no || "").replace(/\D/g, ""), 10) || 0;
      return rollA - rollB;
    });
  ...
  try {
    ...
    if (existingUsername) {
      return res
        .status(400)
        .json({
          error: `Username/ID '${library_card_no}' is already taken by another student.`,
        });
    }
...
studentsRouter.delete("/:id", async (req: Request, res: Response) => {
  try {
    const deleted = await db
      .delete(students)
      .where(eq(students.id, parseInt(req.params.id, 10)))
      .returning();
    if (deleted.length === 0) {
      return res.status(404).json({ error: "Student not found" });
    }
...
// POST /api/students/:id/lift-ban
studentsRouter.post("/:id/lift-ban", async (req: Request, res: Response) => {
  try {
    const studentId = parseInt(req.params.id, 10);
    ...
    if (updated.length === 0)
      return res.status(404).json({ error: "Student not found" });
    res.json({
      message: "Borrowing ban lifted successfully",
      student: updated[0],
    });
  } catch (error: any) {
    res.status(500).json({ error: error.message });
